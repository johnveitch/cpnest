...
class CPNest(object):
    ...
    def __init__(self,
        ...
        if not os.path.exists(resume_file) or resume == False:
            ...
            self.NS = NestedSampler.resume(
                resume_file, self.manager, self.user)
        ...
        for i in range(self.nthreads-nhamiltonian, self.nthreads):
            ...
            if not os.path.exists(resume_file) or resume == False:
                sampler = HamiltonianMonteCarloSampler(self.user,
                                                       ...
                                                       poolsize=poolsize,
                                                       seed=self.seed+i,
                                                       proposal=HamiltonianProposalCycle(
                                                           model=self.user),
                                                       resume_file=resume_file,
                                                       manager=self.manager
                                                       )
    ...
    def get_nested_samples(self, filename='nested_samples.dat'):
        ...
        self.nested_samples = rfn.stack_arrays(
            [s.asnparray()
             for s in self.NS.nested_samples], usemask=False)
        if filename:
    ...
    def plot(self):
        ...
        for n in pos.dtype.names:
            plot.plot_hist(pos[n].ravel(), name=n, filename=os.path.join(
                self.output, 'posterior_{0}.png'.format(n)))
        for n in self.nested_samples.dtype.names:
            plot.plot_chain(self.nested_samples[n], name=n, filename=os.path.join(
    ...
    def worker_sampler(self, producer_pipe, logLmin):
        cProfile.runctx('self.sampler.produce_sample(producer_pipe, logLmin)', globals(
        ), locals(), 'prof_sampler.prof')
    ...
    def profile(self):
        for i in range(0, self.NUMBER_OF_PRODUCER_PROCESSES):
            p = mp.Process(target=self.worker_sampler, args=(
                self.queues[i % len(self.queues)], self.NS.logLmin))
            self.process_pool.append(p)
        for i in range(0, self.NUMBER_OF_CONSUMER_PROCESSES):
            p = mp.Process(target=self.worker_ns, args=(
                self.queues, self.port, self.authkey))
            self.process_pool.append(p)
        for each in self.process_pool:
            each.start()
