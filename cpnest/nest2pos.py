...
def logsubexp(x, y):
    ...
    """
    assert np.all(
        x >= y), 'cannot take log of negative number {0!s} - {1!s}'.format(str(x), str(y))
...
def compute_weights(data, Nlive):
    ...
    log_vols_start = np.cumsum(
        np.ones(len(start_data)+1)*np.log1p(-1./Nlive))-np.log1p(-1./Nlive)
...
def draw_posterior_many(datas, Nlives, verbose=False):
    ...
    if verbose:
        print('Relative weights of input files taking into account their length: {0!s}'.format(
            (str(fracs))))

    posts = [draw_posterior(data, logwt) for (
        data, logwt, logZ) in zip(datas, log_wts, log_evs)]
    if verbose:
