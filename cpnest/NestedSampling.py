...
class _NSintegralState(object):
    ...
    def increment(self, logL, nlive=None):
        ...
        if(logL <= self.logLs[-1]):
            print(
                'WARNING: NS integrator received non-monotonic logL. {0:.3f} -> {1:.3f}'.format(self.logLs[-1], logL))
        if nlive is None:
            nlive = self.nlive
        oldZ = self.logZ
        logt = -1.0/nlive
        ...
        if np.isfinite(oldZ) and np.isfinite(self.logZ) and np.isfinite(logL):
            self.info = np.exp(Wt - self.logZ)*logL + \
                np.exp(oldZ - self.logZ)*(self.info + oldZ) - self.logZ
            if isnan(self.info):
                self.info = 0
...
class NestedSampler(object):
    ...
    def __init__(self, usermodel,
        ...
        self.output_file, self.evidence_file, self.resume_file = self.setup_output(
            output)
    ...
    def consume_sample(self):
        ...
        logLmin = self.get_worst_n_live_points(
            len(self.manager.consumer_pipes))
        logLtmp = []
        for k in self.worst:
        ...
        self.condition = logaddexp(
            self.state.logZ, self.logLmax - self.iteration/(float(self.Nlive))) - self.state.logZ
        ...
        for k in self.worst:
            ...
            while(True):
                ...
                if proposed.logL > self.logLmin.value:
                    ...
                    self.queue_counter = (
                        self.queue_counter + 1) % len(self.manager.consumer_pipes)
                    break
                else:
        ...
        with tqdm(total=self.Nlive, disable=not self.verbose, desc='CPNEST: populate samplers', position=nthreads) as pbar:
            while i < self.Nlive:
                for j in range(nthreads):
                    self.manager.consumer_pipes[j].send(self.model.new_point())
                for j in range(nthreads):
                    while i < self.Nlive:
