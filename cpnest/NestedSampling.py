...
class NestedSampler(object):
	...
	def consume_sample(self):
		...
		for k in self.worst:
			...
			while(True):
				loops += 1
				acceptance, sub_acceptance, jumps, proposed = self.manager.consumer_pipes[self.queue_counter].recv()
			...
			if self.verbose:
				...
				.format(self.iteration, jumps*loops, self.acceptance, k, acceptance, sub_acceptance, self.state.info,\
		...
		with tqdm(total=self.Nlive, disable= not self.verbose, desc='CPNEST: populate samplers', position=nthreads) as pbar:
			while i < self.Nlive:
				...
				for j in range(nthreads):
					while i < self.Nlive:
						acceptance,sub_acceptance,jumps,self.params[i] = self.manager.consumer_pipes[self.queue_counter].recv()
						self.queue_counter = (self.queue_counter + 1) % len(self.manager.consumer_pipes)
