...
class Sampler(object):
    ...
    def reset(self):
        ...
        for n in tqdm(range(self.poolsize), desc='SMPLR {} init draw'.format(self.thread_id),
            ...
            if p.logL is None or not np.isfinite(p.logL):
                print(
                    "Warning: received non-finite logL value {0} with parameters {1}".format(str(p.logL), str(p)))
                print(
                    "You may want to check your likelihood function to improve sampling")
    ...
    def estimate_nmcmc(self, safety=5, tau=None):
        ...
        if self.sub_acceptance == 0.0:
            ...
            self.Nmcmc_exact = (1.0 - 1.0/tau)*self.Nmcmc_exact + \
                (safety/tau)*(2.0/self.sub_acceptance - 1.0)
    ...
    def _produce_sample(self):
        ...
        while True:
            ...
            self.evolution_points.append(p)
            (acceptance, Nmcmc, outParam) = next(
                self.yield_sample(self.logLmin.value))
        ...
        if self.verbose >= 3:
            ...
            sys.stderr.write("Sampler process {0!s}: saved {1:d} mcmc samples in {2!s}\n".format(
                os.getpid(), len(self.samples), 'mcmc_chain_%s.dat' % os.getpid()))
        sys.stderr.write("Sampler process {0!s} - mean acceptance {1:.3f}: exiting\n".format(
            os.getpid(), float(self.mcmc_accepted)/float(self.mcmc_counter)))
        return 0
