...
class GaussianModel(cpnest.model.Model):
    ...
    def log_likelihood(self,p):
        logL = np.sum([-0.5*p['{0}'.format(i)]**2-0.5*np.log(2.0*np.pi) for i in range(self.dim)])
        return logL
...
class GaussianTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.work=cpnest.CPNest(self.model, verbose=2, nthreads=1, nlive=1000, maxmcmc=5000, poolsize=1000, nhamiltonian = 1)
